...
class RecyclingDetectorWithSessions:
    ...
    def __init__(self):
        ...
        self.min_confidence_ratio = 0.75
        self.padding = 20
    ...
    def generate_receipt(self):
        ...
        with st.container():
            ...
            st.write("### Recycling Deposit Receipt")
            ...
            # Display totals by material
            st.write("\nItems by Material Type:")
            for material, data in material_totals.items():
                st.write(f"{material:6} x {data['count']:2} = {data['value']/100:6.2f} €")
            
            st.write("-" * 40)
            st.write(f"Total Items: {total_items}")
            st.write(f"Total Value: {total_value/100:.2f} €")
            st.write("=" * 40)
            ...
            if st.session_state.saved_images:
                ...
                for idx, img in enumerate(st.session_state.saved_images):
                    cols[idx % 3].image(img, caption=f"Item {idx + 1}", width=150)
        ...
        try:
            ...
            while st.session_state.scanning_active:
                ...
                placeholder.image(
                    ...
                    caption=f"Live Feed ({len(captured_data)} readings)"
                )
                        ...
                        if len(captured_data) >= self.required_readings:
                            ...
                            if (most_common[1] / len(texts)) >= self.min_confidence_ratio:
                                ...
                                st.session_state.scanned_items[final_text].append({
                                    ...
                                    'value': pfand_value,
                                    'confidence': (most_common[1]/len(texts))*100
                                })
...
def main():
    ...
    try:
        ...
        with col3:
            if st.button("End Session & Print Receipt"):
                st.session_state.scanning_active = False
